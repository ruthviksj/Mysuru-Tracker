    ...
    <section class="explorer">
      <aside class="panel">
        <div class="panel-head"><span><strong>${filtered().length}</strong> pandals to discover</span>
          <span class="sort"><select id="sort" aria-label="Sort pandals"><option>Most loved</option><option>Newest</option><option>Name A-Z</option></select></span></div>
...
function card(p) {
  ...
  return `<article class="card ${state.selected === p.id ? "selected" : ""}">
    <button class="photo" data-select="${p.id}" aria-label="Focus ${esc(p.name)} on the map"><span class="tag">${typeMarks[p.type]} ${p.type}</span></button>
    <div class="card-body">
...
function refreshMarkers() {
  ...
  markers = filtered().map(p => L.marker([p.lat, p.lng], { icon: pinIcon(p) })
    .addTo(map)
    .bindPopup(`<strong>${esc(p.name)}</strong><br>${esc(p.area)} · ♡ ${p.likes}`)
    .on("click", () => selectPandal(p.id, false)));
}

function pinIcon(p) {
  const color = p.type === "Temple" ? "#cf9a2e" : p.type === "Apartment" ? "#3f6b5a" : p.type === "Other" ? "#97455a" : "#a83c22";
  return L.divIcon({ className: "pin-wrap", html: `<div class="pin" style="background:${color}"><span>${typeMarks[p.type]}</span></div>`, iconSize: [34, 34], iconAnchor: [17, 34], popupAnchor: [0, -32] });
}

function selectPandal(id, recenter = true) {
  state.selected = id;
  const p = withLikes().find(row => row.id === id);
  if (p && map && recenter) map.setView([p.lat, p.lng], Math.max(map.getZoom(), 14));
  const marker = markers.find((m, i) => filtered()[i] && filtered()[i].id === id);
...
function renderAdd() {
  app.innerHTML = `<section class="page route">
    ...
    <form id="pandalForm">
      <section class="form-section"><h2><span class="num">1</span>The pandal</h2>
        ...
        <label>Add a pandal photo <span class="helper">Optional</span>
          <div class="upload"><span class="u-icon">📷</span><strong>Add a pandal photo</strong><span>Take a photo or choose from your gallery</span><small>JPG, PNG, WebP or HEIC · Phone photos are fine</small></div>
        </label>
      </section>
      <section class="form-section"><h2><span class="num">2</span>Where to find Bappa</h2>
        <p class="map-hint">📍 Drag the pin to the exact spot, or type the coordinates.</p>
        <div id="formMap"></div>
        <div class="field-grid">
  ...
  if (typeof L === "undefined") { const el = document.querySelector("#formMap"); if (el) el.innerHTML = `<div class="empty" style="margin:16px">The map could not load. You can still enter coordinates above.</div>`; return; }
  ...
  formMap.on("click", e => { marker.setLatLng(e.latlng); sync(e.latlng); });
